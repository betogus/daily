...
router.get("/:id", async (req: Request, res: Response) => {
  let { id } = req.params;
  try {
    const databaseNews = await database("news")
      .select("*")
      .where("news_id", id)
      .first();
    if (!databaseNews) {
      return res.status(404).send({ error: "Noticia no encontrada" });
    }
    if (databaseNews.tags && typeof databaseNews.tags === "string") {
      databaseNews.tags = databaseNews.tags.split(",");
    } else {
      databaseNews.tags = [];
    }
    res.send(databaseNews);
  } catch (error) {
    res.status(500).send({ error: "Error al obtener noticias" });
  }
});
